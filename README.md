...
`scripts/evaluate.py`：Agent 输出预测后一条命令打分。
...
│   └── evaluate.py                  ← 评测脚本
└── stats/
    ├── mimic3_stats.json            ← MIMIC-III 统计
    └── mimic4_stats.json            ← MIMIC-IV 统计
```
...
```bash
python scripts/build_mimic3_dataset.py   # 约 2 分钟
python scripts/build_mimic4_dataset.py   # 约 2 分钟
```
...
| xzh（G-Memory） | 两个 stats.json | 长尾码清单 = 记忆层最需要补经验的薄弱点 |
